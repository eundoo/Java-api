...
public class MessageNotification2 {
	...
	public MessageNotification2() {
		//HashMap<String, MessageSender> 객체에 MessageSender인터페이스를 구현한 객체를 저장한다.
	...
	public void notice(Customer customer, String from, String title, String content) {
		 ...
		 * 
		 * 향상된 for문을 이용해서 messsageTypes배열에서 messageType으로 하나씩 값을 받아온다.
		 * messageType변수에는 "sms", "katalk", "mail" 중 하나가 저장되어 있다.
		 * messageSender가 참조하는 HashMap객체에서 messageType변수에 저장된 값을 key로
		...
		for (String messageType : messageTypes) {
			//HashMap객체에서 messageType변수에 저장된 값을 key로 MessageSender구현객체를 조회
			MessageSender sender = messageSenders.get(messageType);
			//sender 참조변수가 현재 참고하고 있는 MessageSender류 객체의 sendMessage()메소드가 
			//실행된다.
