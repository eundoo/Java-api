...
public class PropertiesApp1 {
	...
	public static void main(String[] args) throws IOException {
		...
		//		K와 V의 타입으로 String 타입만 사용한다.
		//		K와 V를 설정하지 않는다.
		//		properties파일에 설정된 설정정보를 읽어온다.
		
		//properties 파일에 정의된 설정정보를 처리하는 Properties객체 생성
		...
		//properties파일을 읽어오기올 때 필요한 FileReader객체를 전달한다.
		prop.load(reader);
		...
		System.out.println("데이더 베이스 연결관련 설정정보");
		...
		System.out.println("###파일 업로드 관련 설정정보");
		System.out.println("최대 업로드 사이드" + value5);
		System.out.println("업로드 파일 저장경로" + value6);
	}
